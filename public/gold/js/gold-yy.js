...
function queryEntrust(msg, dataTime) {
  pbE.SYS().stopLoading();
  ...
  if (CONTENTS1.length == 0) {
    ...
    $('#alert').addClass('hide');
    var CONTENTS = pbUtils.sortDlEn(CONTENTS1, '159');
    ...
    ReactDOM.render(React.createElement(TransferContents, { contents: CONTENTS, history: true }), document.getElementById('contents'));
  }
}
...
$(function () {
  ...
  ReactDOM.render(React.createElement(NavHeader, { name: "预约查询" }), document.getElementById('nav-header'));
  ReactDOM.render(React.createElement(TimeTitle, { times: TIMES, transfer: true }), document.getElementById('time-title'));
  ReactDOM.render(React.createElement(TransferTitle, null), document.getElementById('title'));
  ...
  var option = {
    callbacks: [{fun: 9317, module: 90002, callback: function (msg) {queryEntrust(msg, dataTime);}}
               ],
