  ...
  if (msg.jData['1'] < 0) {
    ...
    if (data.length == 0) {
      ...
      for (var i = 0, m = data.length; i < m; i++) {
        for (var j = 0, n = conf.rz.title.length; j < n; j++) {
          if (data[i][conf.rz.title[j].code]) {
            $('#title' + j + ' .content').append('<p>' + data[i][conf.rz.title[j].code] + '</p>');
          } else {
...
$(function () {
  ...
  var option = {
    callbacks: [{ fun: parseInt(conf.rz.functionNO), module: 90002, callback: function (msg) {
        creditDebt(msg);
      }
    }],
    ...
    refresh: function () {
      ...
      pbE.WT().wtGeneralRequest(CID, parseInt(conf.rz.functionNO), JSON.stringify({}));
    },
  ...
  for (var i = 0; i < conf.rz.title.length; i++) {
    $('#title').append('<div class="col-my-1" id="title' + i + '"><p class="title">' + conf.rz.title[i].name + '</p><div class="content"></div></div>');
  }
