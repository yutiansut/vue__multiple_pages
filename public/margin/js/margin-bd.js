  ...
  if (msg.jData['1'] < 0) {
    ...
    if (data.length == 0) {
      ...
      if (clickFlag == 1) {
        ...
        $('#more').addClass('hide');
        $('#indicator').addClass('hide');
        for (var j = 0; j < conf.bd.title.length; j++) {
      ...
      for (var i = 0, m = data.length; i < m; i++) {
        for (var j = 0, n = conf.bd.title.length; j < n; j++) {
          if (data[i][conf.bd.title[j].code]) {
            $('#title' + j + ' .content').append('<p>' + data[i][conf.bd.title[j].code] + '</p>');
          } else {
...
$(function () {
  ...
  $('.searchButton').click(function () {
    if (clickFlag == 0) {
      ...
      if (!market) {
        ...
        pbE.WT().wtGeneralRequest(CID, parseInt(conf.bd.functionNO), JSON.stringify(par));
      }
    }
  });
  ...
  if (!isApp) {
    ...
    /*conf = {
      "bd": {
        "barName": "查询标的证券",
        "search": {'flag': '1', 'placeholder': '请输入您要查询的标的证券', 'item1': '#title3', 'item2':'#title4'},
        "title": [{'name':'市场名称','code':'55'}, 
                  {'name':'证券代码','code':'512'}, 
                  {'name':'证券名称','code':'513'}, 
                  {'name':'融资允许标志','code':'637'}, 
                  {'name':'融券允许标志','code':'639'}, 
                  {'name':'折算系数','code':'640'}, 
                  {'name':'折算比例','code':'588'}, 
                  {'name':'融资比例','code':'642'}, 
                  {'name':'融券比例','code':'643'}, 
  ...
  var option = {
    callbacks: [{ fun: parseInt(conf.bd.functionNO), module: 90002, callback: function (msg) {
        bdQuery(msg);
      }
    }],
    ...
    refresh: function () {
      ...
      pbE.SYS().startLoading();
      pbE.WT().wtGeneralRequest(CID, parseInt(conf.bd.functionNO), JSON.stringify(data));
    },
  ...
  for (var i = 0; i < conf.bd.title.length; i++) {
    $('#title').append('<div class="col-my-1" id="title' + i + '"><p class="title">' + conf.bd.title[i].name + '</p><div class="content"></div></div>');
  }
  ...
  window.onscroll = function () {
    ...
    if ($(document).height() <= totalheight) {
      if (page == pageNum) {
        ...
        start += end;
        if (page == pageNum - 1) {
          data1 = { '9': start.toString(), '10': (total - 100 * (pageNum - 1)).toString() };
        } else {
          data1 = { '9': start.toString(), '10': end.toString() };
        }
        requestNum++;
        pbE.SYS().startLoading();
        ...
        if (requestNum == 2 || requestNum > 2 && pageFlag == 0) {
          pageFlag = 1;
          pbE.WT().wtGeneralRequest(CID, parseInt(conf.bd.functionNO), JSON.stringify(data1));
        }
      }
    }
  };
